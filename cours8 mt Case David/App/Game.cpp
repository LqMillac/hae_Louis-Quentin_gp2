...
void Game::update(double dt){
	for (auto it = evec.begin(); it != evec.end();) {
		...
		ent->update(dt);
		it++;
		if (ent->getState() == EntityState::ES_IDLE && oldState!= ES_IDLE)
		{
			RestartPathFinding();
			oldState = ent->getState();
		}
		else
		{
			oldState = ent->getState();
...
void Game::RestartPathFinding()
{
	...
	int size = 30;
	for (int x = player->cx - 10; x < player->cx + 10; x++) {
		for (int y =player->cy - 10; y < player->cy + 10; y++) {
			if (!willCollide(x, y))
				graph.push_back(Vector2f(x, y));
		}
	}
	printf("len:%d\n", graph.size());
	dijo.compute(graph, Vector2f(player->cx, player->cy));
	printf("computed\n");
}
